            ...
            <InputGroupAddon align="inline-end">
                {showTrigger && (
                    <ComboboxTrigger
                        ...
                        className="group-has-data-[slot=combobox-clear]/input-group:hidden size-6 rounded-[calc(var(--radius)-3px)] flex items-center justify-center hover:bg-accent"
                        disabled={disabled}
                    />
                )}
...
function ComboboxChip({
    ...
    return (
        ...
        >
            {children}
            {showRemove && (
                <ComboboxPrimitive.ChipRemove className="-ml-1 opacity-50 hover:opacity-100" data-slot="combobox-chip-remove" render={<Button variant="ghost" size="icon" className="h-5 w-5"><XIcon className="pointer-events-none size-3" /></Button>} />
            )}
        </ComboboxPrimitive.Chip>
    )
}
