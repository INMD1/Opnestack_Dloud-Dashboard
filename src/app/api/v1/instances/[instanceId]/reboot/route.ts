...
export async function POST(
    ...
    try {
        ...
        const { data, error, response } = await skylineClient.POST(
            `/api/v1/instances/${encodeURIComponent(instanceId)}/reboot`,
            {}
        );
        ...
        console.error("Reboot Instance API error:", err);
        return jsonResponse({ message: "Reboot Instance API failed" }, 500);
    }
}
